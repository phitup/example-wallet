    ...
    switch (this.associationType) {
      ...
      case AssociationType.LocalFromApp:
        if (clientIdentityUri != null) {
          ...
          console.log('ccccc verified : ', verified);
          if (verified) {
      ...
      if (this.associationType != AssociationType.LocalFromBrowser) {
        ...
        console.warn(
          `Unexpected character '${
            authorizationScope[AssociationType.LocalFromBrowser.length]
          }' in scope; expected '${ClientTrustUseCase.SCOPE_DELIMITER}'`,
        );
        return false;
      } else {
        console.debug(
  ...
  private getAssociationType(associationUri: string): AssociationType {
    ...
    if (parsedUri.endsWith(ClientTrustUseCase.LOCAL_PATH_SUFFIX)) {
      if (this.callingPackage != null) {
        return AssociationType.LocalFromApp;
      } else {
        return AssociationType.LocalFromBrowser;
      }
